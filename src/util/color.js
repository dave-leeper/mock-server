...
class Color {
    ...
    static xlateRGB255ColorToNormalizedColor(in255Color) {
        if (255 < in255Color) in255Color = 255;
        let f255Color = in255Color * 1.00;
        return f255Color / 255.0;
    }

    static xlateRGBNormalizedColorTo255Color(inNormalizedColor) {
        if (0.0 > inNormalizedColor) inNormalizedColor = 0.0;
        if (1.0 < inNormalizedColor) inNormalizedColor = 1.0;
    ...
    static xlateRGB255ToHSV360(r, g, b) {
        let min3 = (a, b, c) => { return (a < b)?((a < c)? a : c) : ((b < c)? b : c ); };
        let max3 = (a, b, c) => { return (a > b)?((a > c)? a : c) : ((b > c)? b : c ); };
        let hsv = {};
        let rgb = { r: r, g: g, b: b };
        let max = max3(rgb.r, rgb.g, rgb.b);
        let dif = max - min3(rgb.r, rgb.g, rgb.b);
        hsv.saturation = (0.0 === max)? 0 : (100 * dif / max);
        if (0 === hsv.saturation) hsv.hue = 0;
    ...
    static xlateHSV360ToRGB255(h, s, v) {
        let hsv = { hue: h, saturation: s, value: v };
        let rgb = {};
        if (0 === hsv.saturation) {
    ...
    static fromString(inColorString) {
        ...
        if ('transparent' === inColorString) return Color.getTransparentColor();
        let aRGB = inColorString.replace(/rgb\(/, '').replace(/\)/g, '').replace(/ /g, '').split(',');
    ...
    static fromStringWithAlpha(inColorString) {
        if (!inColorString) return null;
        if ('transparent' === inColorString) return Color.getTransparentColor();
        let aRGB = inColorString.replace(/rgba\(/, '').replace(/\)/g, '').replace(/ /g, '').split(',');
    ...
    static fromFilterString(inColorString) {
        if (!inColorString) return null;
        let color = new Color();
        let aFilter = inColorString.split('=');
        if ((!aFilter) || (2 > aFilter.length)) return;
        let strFilter = aFilter[1].replace(/\)/g, '').replace(/ /g, '');
    ...
    static fromHSLString(inColorString) {
        if (!inColorString) return null;
        if ('transparent' === inColorString) return Color.getTransparentColor();
        let aHSL = inColorString.replace(/hsl\(/, '').replace(/%/g, '').replace(/\)/g, '').replace(/ /g, '').split(',');
        let aRGB = Color.xlateHSL360ToRGB255(aHSL[0], aHSL[1], aHSL[2]);
    ...
    static fromHSLAString(inColorString) {
        if (!inColorString) return null;
        if ('transparent' === inColorString) return Color.getTransparentColor();
        let aHSLA = inColorString.replace(/hsla\(/, '').replace(/%/g, '').replace(/\)/g, '').replace(/ /g, '').split(',');
        let aRGB = Color.xlateHSL360ToRGB255(aHSLA[0], aHSLA[1], aHSLA[2]);
    ...
    static fromHexString(inColorString) {
        if (!inColorString) return null;
        if ('transparent' === inColorString) return Color.getTransparentColor();
        let aRGB =  Color.xlateHexToRGB255(inColorString);
        return new Color(aRGB[0], aRGB[1], aRGB[2]);
    }
    ...
    static getNormalizedColor(color) {
        let oNormalized = new Color(color.r, color.g, color.b, color.a, color.transparent);
        if (!color.normalized) {
    ...
    static get255Color(color) {
        let o255 = new Color(color.r, color.g, color.b, color.a, color.transparent);
        if (color.normalized) {
     ...
     */
    static clamp(color) {
        let newColor = Color.clone(color);
        if (true !== newColor.transparent) newColor.transparent = false;
        if (true !== newColor.normalized) newColor.normalized = false;
        let max = (newColor.normalized)? 1.0 : 255;
     ...
     */
    static invert(color) {
        let newColor = Color.get255Color(color);
    ...
    static blend(color1, color2, inBlendOperation) {
        ...
        if (Color.blendOperation.CROSS === inBlendOperation) {
            ...
            color2Normalized.r = (color2Normalized.r + color1Normalized.r);
            color2Normalized.g = (color2Normalized.g + color1Normalized.g);
            color2Normalized.b = (color2Normalized.b + color1Normalized.b);
            color2Normalized.a = (color2Normalized.a + color1Normalized.a);
        } else if (Color.blendOperation.ADDITIVE_ALPHA === inBlendOperation) {
            color2Normalized.r = (color2Normalized.r + color1Normalized.r * color1Normalized.a);
            color2Normalized.g = (color2Normalized.g + color1Normalized.g * color1Normalized.a);
            color2Normalized.b = (color2Normalized.b + color1Normalized.b * color1Normalized.a);
            color2Normalized.a = (color2Normalized.a + color1Normalized.a * color1Normalized.a);
    ...
    static addColor(color1, color2) {
        let newColor = Color.clone(color1);
        newColor.r += color2.r;
        newColor.g += color2.g;
        newColor.b += color2.b;
        return newColor;
    ...
    constructor(inRed, inGreen, inBlue, inAlpha, inTransparent, inNormalized) {
        this.r = inRed? inRed : 0;
        this.g = inGreen? inGreen : 0;
        this.b = inBlue? inBlue : 0;
        this.a = inAlpha? inAlpha : ((0 === inAlpha)? 0 : 1.0);
        this.transparent = inTransparent? inTransparent : false;
        this.normalized = inNormalized? inNormalized : false;
    }
    ...
    isEqual(inColor, inColorTolerance, inAlphaTolerance) {
        let colorTolerance = (inColorTolerance)? inColorTolerance : 0;
        let alphaTolerance = (inAlphaTolerance)? inAlphaTolerance : colorTolerance;
    ...
    toHSLString() {
        if (this.transparent) return 'transparent';
        let color = Color.get255Color(this);
        let hsl = Color.xlateRGB255ToHSL360(color.r, color.g, color.b);
