...
class DeleteFavorite {
    do(params) {
        ...
        if (!body.id) {
            let message = Strings.format(I18n.get( Strings.ERROR_MESSAGE_FAVORITE_DELETE_FAILED ), body.username);
            if (Log.will(Log.ERROR)) Log.error(message);
            inReject && inReject({ status: 400, send: message});
            return;
        }
        if (!body.issue) {
            let message = I18n.get( Strings.ERROR_MESSAGE_FAVORITE_DELETE_FAILEDs );
            if (Log.will(Log.ERROR)) Log.error(message);
            inReject && inReject({ status: 400, send: message});
            return;
        }
        ...
        if (Files.existsSync(filePath)) {
            ...
            for (let i = favorites.length - 1; i >= 0; i--) {
                if (favorites[i].id.toUpperCase() === newFavorite.id.toUpperCase() && favorites[i].issue === newFavorite.issue ) {
                    let successCallback = () => {
                        let message = I18n.get( Strings.SUCCESS_MESSAGE_FAVORITE_DELETED );
