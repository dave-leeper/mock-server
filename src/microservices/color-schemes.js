...
class ColorSchemes {
    do(params) {
        return new Promise (( inResolve, inReject ) => {
            ...
            if (!body.color) {
                let message = I18n.get( Strings.ERROR_NO_COLOR );
                if (Log.will(Log.ERROR)) Log.error(message);
                inReject && inReject({ status: 400, send: message});
                return;
            }
            let colorRGB = Color.fromHexString(body.color);
            let color = new Color(colorRGB.r, colorRGB.g, colorRGB.b);
            let complimentaryColor = Color.getComplimentaryColorScheme(color);                  // Array of 2 colors. 1st is original color.
            let splitComplimentaryColor = Color.getSplitComplimentaryColorScheme(color);        // Array of 3 colors. 1st is original color.
            let triadColor = Color.getTriadColorScheme(color);                                  // Array of 3 colors. 1st is original color.
            let analogousColor = Color.getAnalogousColorScheme(color);                          // Array of 3 colors. 1st is original color.
            let rectangleColor = Color.getRectangleColorScheme(color);                          // Array of 4 colors. 1st is original color.
            let squareColor = Color.getSquareColorScheme(color);                                // Array of 4 colors. 1st is original color.
            let result = { originalColor: { r: color.r,  g: color.g, b: color.b } };
